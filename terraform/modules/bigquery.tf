...
resource "google_bigquery_table" "kokiri_1d" {
  dataset_id  = "${var.environment}"
  table_id    = "${google_bigquery_table.kokiri.table_id}_1d"
  description = "Kokiri requests for the past day"
  ...
  view {
    query = <<-EOF
            ...
            WHERE
              _PARTITIONTIME is null OR
              _PARTITIONTIME BETWEEN TIMESTAMP(UTC_USEC_TO_DAY(NOW() - 1 * 60 * 60 * 24 * 1000000))
              AND TIMESTAMP(UTC_USEC_TO_DAY(CURRENT_TIMESTAMP()));
            EOF
  }
}
