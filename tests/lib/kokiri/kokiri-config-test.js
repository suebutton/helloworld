...
describe('lib/kokiri/kokiri-config', function() {
  describe('#merchantIdByUrl', function() {
    beforeEach(function() {
      ...
      this.config = createConfig(supportedMerchants, [], [], [], [], approvals);
    });
  ...
  describe('#merchantIdByUrl bttn URLs', function() {
    ...
    it('returns a merchant id for a known subdomain', function() {
      ...
      assert.equal(
        this.config.merchantIdByUrl('http://track.bttn.io/bloop', PUBLISHER_ID),
        'org-3573c6b896624279'
      );
    ...
    it('returns null for unknown subdomains', function() {
      assert.equal(
        this.config.merchantIdByUrl('http://bloop.bleep.bttn.io', PUBLISHER_ID),
        null
      );
  ...
  describe('#merchantIdByUrl affiliate URLs', function() {
    beforeEach(function() {
      ...
      const supportedAffiliates = [
        ...
        {
          hostname: 'linksynergy.com',
          query_url_keys: [{ key: 'wurl' }, { key: 'url' }, { key: 'kurl' }],
        },
        {
  ...
  describe('#destinationUrlFromBttnioUrl', function() {
    beforeEach(function() {
      ...
      this.config = createConfig([], [], [], [], webToAppMappings, approvals);
    });
    ...
    it('returns a destination for track bttnio links', function() {
      assert.deepEqual(
        this.config.destinationUrlFromBttnioUrl('https://track.bttn.io/bloop'),
        'http://bloop.com/'
      );
    ...
    it(`returns null for links that don't match`, function() {
      assert.equal(
        this.config.destinationUrlFromBttnioUrl('https://bloop.bleep.bttn.io'),
        null
      );
      ...
      assert.equal(
        this.config.destinationUrlFromBttnioUrl('https://track.bttn.iop/bloop'),
        null
      );
  ...
  describe('#createMockBuilder', function() {
    ...
    it('allows the creation of a builder for an unapproved merchant', function() {
      this.config.createMockBuilder('org-3573c6b896624279');
    });
  });
  ...
  describe('#supportedAffiliateByUrl', function() {
    ...
    it('returns the affiliate matched by a url', function() {
      assert.deepEqual(
        this.config.supportedAffiliateByUrl('https://click.linksynergy.com'),
    ...
    it('returns null if not found', function() {
      assert.deepEqual(
        this.config.supportedAffiliateByUrl('https://bloop.linksynergy.com'),
        null
      );
    });
  });
  ...
  describe('#shouldRedirectByUrl', function() {
    ...
    it('returns true for links that match', function() {
      ...
      assert(
        this.config.shouldRedirectByUrl('http://groupon.com/redirect/123/bleep')
      );
      assert(
        this.config.shouldRedirectByUrl('http://bloopon.com/visit/123/456')
      );
      assert(
        this.config.shouldRedirectByUrl('http://bloopon.com/visit/123/456/blop')
      );
    });
  });
