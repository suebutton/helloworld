const { get, isString, extend } = require('lodash');
...
const { FALLBACK_PARAM, FALLBACK_WEB } = require('./constants');
...
class KokiriAdapter {
  ...
  static getAppAction(builder, url, platform, attributionToken) {
  ...
  static getWebAction(builder, url, platform, attributionToken) {
    return builder.webAction(
      builder.destinationFromUrl(url),
      platform,
      attributionToken
    );
  ...
  static supportsWebToAppWithInstall(webAction) {
    const webToAppLink = get(webAction, 'app_link');

    if (!isString(webToAppLink)) {
      return false;
    }

    const { query } = parse(webToAppLink, true);
    return get(query, FALLBACK_PARAM) !== FALLBACK_WEB;
  }

  static supportsToApp(action) {
    return !!(action && action.app_link);
  }

  static supportsToWeb(action) {
    return !!(action && action.browser_link);
  }

  /**
   * Generates a support matrix for a passed-in builder.
   * Can be used to generate matrices for unapproved partners via mock builder.
   *
   * @param {() => LinkBuilder} A 0-arity function that returns a LinkBuilder
   *   instance
   * @param  {string} url The url in question
   * @param  {string} platform "ios" or "android"
   * @return {Object} The support matrix
   */
  static supportMatrixFromBuilder(getBuilder, url, platform) {
    let appToApp = false;
    let appToWeb = false;
    let webToApp = false;
    let webToAppWithInstall = false;
    let webToWeb = false;

    try {
      const builder = getBuilder();
      const mockSourceToken = 'srctok-XXX';
      ...
      const webAction = KokiriAdapter.getWebAction(
        builder,
        url,
        platform,
        mockSourceToken
      ...
      appToApp = KokiriAdapter.supportsToApp(appAction);
      appToWeb = KokiriAdapter.supportsToWeb(appAction);
      webToApp = KokiriAdapter.supportsToApp(webAction);
      webToAppWithInstall = KokiriAdapter.supportsWebToAppWithInstall(
        webAction
      );
      webToWeb = appToWeb;
    } catch (e) {}
  ...
  supportMatrix(url, publisherId, platform) {
    ...
    return KokiriAdapter.supportMatrixFromBuilder(
      () => kokiriConfig.createBuilderByUrl(publisherId, url),
      url,
      platform
    );
  }
  ...
  webAction(url, publisherId, platform, attributionToken, ctx, originalUrl) {
    ...
    try {
      ...
      merchantId = kokiriConfig.merchantIdByUrl(url);
      const builder = kokiriConfig.createBuilderByUrl(publisherId, url);
      webAction = KokiriAdapter.getWebAction(
        builder,
        url,
        platform,
        attributionToken
  ...
  exampleLinks(merchantId) {
    return EXAMPLE_LINKS.filter(example => example.merchant_id === merchantId);
  }
  ...
  appLinkingSupport(platform, merchantId, url) {
    ...
    const supportMatrix = KokiriAdapter.supportMatrixFromBuilder(
      () => kokiriConfig.createMockBuilder(merchantId),
      url,
      platform
    );
    ...
    return {
      webToApp: false,
      appToApp: supportMatrix.appToApp,
    };
  }

  /**
   * Returns the baseline flow capabilities of a merchant per platform.
   *
   * @param  {string} platform
   * @param  {string} merchantId
   * @return {string}
   */
  baselineSupport(platform, merchantId) {
    const kokiriConfig = this.kokiriConfig();
