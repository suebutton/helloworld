...
class KokiriAdapter {
   ...
   * @private
   */
  incrementEvent(status, publisher, merchant) {
   ...
   * @private
   */
  onSuccess(publisher, merchant) {
    this.incrementEvent('success', publisher, merchant);
  }

  /**
   * @private
   */
  onError(publisher, merchant) {
    this.incrementEvent('error', publisher, merchant);
  }
  ...
  appAction(url, publisherId, platform, attributionToken, ctx, originalUrl) {
    ...
    try {
      ...
      this.onSuccess(publisherId, merchantId);
      return appAction;
    } catch (e) {
      this.onError(publisherId, merchantId);
      return null;
    } finally {
