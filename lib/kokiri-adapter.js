...
class KokiriAdapter {
  ...
  static supportsWebToAppWithInstall(webAction) {
    const appLinkParams = parse(webAction.app_link, true).query;
    const webToAppWithInstall = appLinkParams[FALLBACK_PARAM] !== FALLBACK_WEB;
    return webToAppWithInstall;
  }
  ...
  supportMatrix(url, publisherId, platform) {
    ...
    webToWeb = appToWeb;
    webToApp = !!(webAction && webAction.app_link);
