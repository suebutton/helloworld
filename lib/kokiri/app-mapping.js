...
function mapDestination(ctx, knownMappings, destination, platform) {
  ...
  return knownMappings.reduce(
    (acc, candidate) => {
      ...
      return {
        match: true,
        value: { ...destination, ...appDestination },
      };
    },
...
/**
 * Higher-order matcher function for matching a pathname in the destination.
 *
 * Invoking with a pattern string that describes the pathname will return
 * a function that returns an Object when passed a matching destination.  The
 * returned Object contains all matched parameters from the pattern string.
 *
 * Pattern strings are interpretted by
 * [path-to-regexp](https://github.com/pillarjs/path-to-regexp), the same
 * library that handles koa-router pathname matching.  They support named
 * parameters that will be the keys of the returned Object,
 * optional parameters, and parameter-specific regexs, among others.
 *
 * e.x.
 ...
 *   // returns { 'one': '1', 'two': '2', 'three': '3' }
 *
 * @param  {String} pathString A string describing the pattern
 * @return {Function} A matcher function
 */
function matchPathname(pathString) {
  const keys = [];
  const re = pathToRegexp(pathString, keys);

  return destination => {
    const match = re.exec(destination.pathname);

    if (!match) {
      return null;
    }

    return zipObject(keys.map(k => k.name), match.slice(1));
  };
}
