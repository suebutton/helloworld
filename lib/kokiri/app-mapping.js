 ...
 *
 * It can be used with #mapWebToAppDestination and isSupportedAppDestination:
 *
 ...
 * * `destination`: An object or arbitrary keys which will be merged on top of
 *   the web destination.  If the value of a given key is a function, it will be
 ...
 */
function mapWebToAppDestination(ctx, knownMappings, destination, platform) {
  if (!knownMappings) {
    return destination;
  }

  return knownMappings.reduce((acc, candidate) => {
    if (acc) {
      return acc;
    }
    ...
    return extend({}, destination, appDestination);
  }, null);
}

/**
 * Returns whether or not a destination is well known on the target platform,
 * given a set of knownMappings.
 *
 * @param  {Object} ctx
 * @param  {Array<Object>}  knownMappings
 * @param  {Object}  destination
 ...
 * @return {boolean}
 */
function isSupportedAppDestination(ctx, knownMappings, destination, platform) {
  if (!knownMappings) {
    return false;
  }
...
function matchPlatform(desiredPlatform) {
  return (destination, platform) => (desiredPlatform === platform ? {} : null);
}
 ...
 * null, the entire sequence will return null.  If all match, any returned
 * objects from the individual matchers will be merged together.
 *
