...
/**
 * Attempts to normalize a Url, given potentially noisy input from
 * a partner.
 *
 * Currently enforces that a protocol is specified on the URL by prepending one
 * (http) if not present.
 *
 * @param  {string} url
 * @return {string}
...
function normalizeUrl(url) {
  return url.match(PROTOCOL_REGEX) === null ? `https://${url}` : url;
}
 ...
 *
 * @param  {array} queryBlacklist array of blacklisted query parameters
 * @param  {string} url
 * @return {string}
 */
...
/**
 * Returns a Cache key for an incoming wirecutter url.
 *
 * @param  {string} url
 * @return {string}
 */
function urlCacheKeyWC(url) {
  const { protocol, hostname, pathname } = parseUrl(url);

  const match = pathname.match(/^\/link\/[0-9]+/);

  if (!protocol || !hostname || !match) {
    return url;
  }
