...
const { extend, zip, every, omit, isString, pickBy } = require('lodash');
...
function formatUrl(urlObject, fixPathname = true) {
  ...
  return format(extend({}, urlObject, { pathname, query: compact(query) }));
}
...
/**
 * Attributes a url with the provided attribution token at provided key.
 *
 * @param  {string} url
 * @param  {string} attributionToken
 * @param  {string} btnRefKey
