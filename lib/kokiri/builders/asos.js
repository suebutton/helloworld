...
const { formatUrl, joinPathname, cleanPathname } = require('../lib');
const {
  matchHomepage,
  matchPathname,
  composeMatches,
...
Asos.AppMappings = [
  ...
  {
    match: matchPathname('/:topcategory/:subCategory/prd/:productId([0-9]+)'),
    destination: {
      pathname: '/product',
  ...
  },
  {
    match: composeMatches(
      matchPathname(
        '/:base_category_slug/:sub_category_slug?/:sub_category_slug2?/cat/'
      ),
      matchQuery({ cid: /[0-9]+/ })
    ),
    destination: {
