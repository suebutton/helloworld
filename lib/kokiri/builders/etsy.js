...
const UTM_CONTENT_PASSTHROUGH_WHITELIST = new Set([
...
class Etsy extends LinkBuilder {
  ...
  utmcontent(destination) {
    if (UTM_CONTENT_PASSTHROUGH_WHITELIST.has(this.publisherId)) {
      return get(destination.query, 'utm_content', 'button');
    }
  ...
  query(destination) {
    return {
      ...
      ...{
        ...
        utm_content: this.utmcontent(destination),
      },
    };
  }
