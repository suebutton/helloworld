...
class LinkBuilder {
   ...
   *
   * TODO(will) this needs cleanup / better abstracting with kokiri
   *
   * @param  {string} url
  ...
  destinationFromUrl(url) {
    ...
    unwrappedUrl = this.config.destinationUrlFromBttnioUrl(url);

    if (unwrappedUrl !== null) {
      return this.getDestinationFromUrl(unwrappedUrl);
    }

    unwrappedUrl = this.config.destinationUrlFromAffiliateUrl(url);

    if (unwrappedUrl !== null) {
      return this.getDestinationFromUrl(unwrappedUrl);
    }

    if (this.config.supportedAffiliateByUrl(url)) {
      return this.getDestinationFromUrl('');
    }

    return this.getDestinationFromUrl(url);
  }

  /**
   * Maps a web destination object to platform-specific app destination object,
   * or null if that destination is not supported by the target platform.
   *
   * @private
  ...
  mapWebDestinationToApp(destination, platform) {
    return mapWebToAppDestination(
      this,
      this.constructor.KnownMappings,
      destination,
      platform
    );
  }
  ...
  /**
   * Returns whether or not a destination is supported by the provided
   * platform.
   *
   * N.b. This method shouldn't be trusted, as specific builders (like Amazon)
   * might feature internal blacklists not known to the KnownMappings framework.
   *
   * TODO(will,sean): either make this method reliable or remove it.
   *
   * @param  {Object}  destination
   * @param  {string}  platform
   * @return {boolean}
   */
  isSupportedAppDestination(destination, platform) {
    return isSupportedAppDestination(
      this,
      this.constructor.KnownMappings,
      destination,
      platform
  ...
  /**
   * Returns a track.bttn.io/<partner> link.
   *
   * This method must not mutate destination.  A ?btn_ref query param will be
   * added to all outgoing links.
   *
   * @param {Object} destination
   * @param {string} [attributionToken] If falsey, the link will be "statically"
   *   affiliated and use the publisher organization id as the affiliation
   *   parameter.
   * @return {?string} The universal link
   */
  universalLink(destination, attributionToken) {
    ...
    if (!attributionToken) {
      attributionToken = this.publisherId;
    }
    ...
    return formatUrl({
      protocol: 'https',
      hostname: 'track.bttn.io',
      pathname: joinPathname([partnerSubdomain, pathname]),
      query: attributeQuery(query, attributionToken),
      hash,
      slashes: true,
    });
  }
...
LinkBuilder.KnownMappings = [{ match: true }];
