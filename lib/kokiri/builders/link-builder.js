...
class LinkBuilder {
   ...
   * Compares the value against one from par par
   * For testing, this will log to sentry when they mismatch
   *
   * @param  {string} name, name of the parameter
   * @param  {string} expectedValue, the value our configuration says we should have
   * @return {string} the value for this name/partner pairing
   */
  getPartnerValue(name, expectedValue) {
    const calculatedValue = this.config.getPartnerValue(
      name,
      this.merchantId,
      this.publisherId
    );
    if (expectedValue !== calculatedValue) {
      if (this.config._errorLogger) {
        this.config._errorLogger.logError(
          `Mismatched value. Expected ${expectedValue}, got ${calculatedValue}`
        );
      }
    }
  ...
  /**
   * TODO: this should return null
   * TODO: make universal link writing symmetric with get{App,Browser}Link
   *
