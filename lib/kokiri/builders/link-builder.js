...
class LinkBuilder {
   ...
   * also supports Universal Linking.  The strategy is to try and route to a
   * link on the merchant's domain that is not a Universal Link to try and keep
   * users in their browser. I'd like to consider a more holisitic
   * approach to handling this sort-of thing in the future.
   *
   * Maps a web destination object to platform-specific browser-based
   * destination object, or null if that destination is not supported by the
   * target platform.  This mapping will only be applied for web to web links--
   * app to web links that target our SDKs webview will not be affected.
   *
   * @private
   * @param  {Object} destination
   * @param  {string} platform
   * @return {?Object}
   */
  mapWebDestinationToBrowser(destination, platform) {
    return mapDestination(
      this,
      this.constructor.BrowserMappings,
      destination,
      platform
    );
  }
  ...
  /**
   * Convenience method that composes extracting a destination from a url and
   * and generating an action based on it.
   *
   * @param  {string} url
   * @param  {string} platform
   * @param  {string} attributionToken
  ...
  webAction(destination, platform, attributionToken) {
    ...
    const webDestination = this.mapWebDestinationToBrowser(
      destination,
      platform
    );
  ...
  getDestinationFromUrl(url) {
    return {
      url,
      ...pick(parseUrl(url), ['hostname', 'pathname', 'query', 'hash']),
    };
  }
...
LinkBuilder.BrowserMappings = [{ match: true }];
