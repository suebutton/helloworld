...
class Zulily extends LinkBuilder {
  ...
  getButtonUniversalLink(appDestination, platform, attributionToken) {
    ...
    return formatButtonUniversalUrl('zulily', {
      pathname,
      query: this.query(appDestination, attributionToken),
      hash,
    });
...
Zulily.AppMappings = [
  {
    match: matchHomepage,
    destination: {
      pathname: '/action.show/view/newToday',
    },
  },
  {
    match: composeMatches(matchIOS, matchProductPage),
    destination: {
      pathname: '/action.show/product',
  ...
  },
  {
    ...
    destination: {
      pathname: match => `/action.show/view/product/${match.productId}`,
      query: (match, prevQuery) => ({
        ...prevQuery,
...
Zulily.UniversalMappings = Zulily.AppMappings;
