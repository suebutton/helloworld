 ...
 * AE, AU, BE, DE, ES, FR, IE, IT, NL, NZ, PL, UK, US
 *
 */
...
class Groupon extends LinkBuilder {
  hostname(destination) {
    if (destination.hostname in HOSTNAME_TO_REGION_MAP) {
  ...
  getDestinationFromUrl(url) {
    ...
    if (normalizedHostname in Groupon.trackingHostnameToRegionMap) {
      ...
      return extend({}, baseDestination, {
        hostname: Groupon.trackingHostnameToRegionMap[normalizedHostname],
      });
    }
...
Groupon.trackingHostnameToRegionMap = invert(HOSTNAME_TO_TRACKING_HOSTNAME_MAP);
