...
const CAMPID_PASSTHROUGH_WHITELIST = {
  // Ibotta campid values to be updated whence available.
  'org-2d432a88b9bb8bda': new Set(['123', '234']), // Ibotta
};
...
class Ebay extends LinkBuilder {
  ...
  programid(hostname) {
    return PROGRAM_IDS[normalizeHostname(hostname.replace(/^m\./, ''))] || null;
  }

  redirectHostname(destination) {
    const { hostname } = destination;
    return hostname.replace(/^m\./, 'www.') || 'www.ebay.com';
  }
