...
class KokiriConfig {
  ...
  /**
   * Returns an instantiated LinkBuilder which ignores approvals
   * @param {string} merchantId
   * @return {LinkBuilder} the merchant-specific LinkBuilder instance
   */
  createMockBuilder(merchantId) {
    if (!merchantId) {
      throw new KokiriError(`Invalid merchantId: ${merchantId}`);
    }
    return createBuilder(this, null, merchantId);
  }
  ...
  destinationUrlFromBttnioUrl(url) {
    ...
    return format({
      ...destination,
      ...{ protocol: 'http', hostname: externalHostname },
    });
  }
   ...
   * @param  {string} partner Organization ID of the partner who owns the value
  ...
  getPartnerValue(name, owner, partner, fallback) {
    const parameter = this._partnerParameters.find(
      pp => pp.name === name && pp.organization === owner
    );
    ...
    const { id, default_value: defaultValue } = parameter;

    const partnerValue = this._partnerValues.find(
      pv => pv.partner_parameter === id && pv.organization === partner
    );

    return partnerValue ? partnerValue.value : defaultValue;
  }
}
